...
`-e`（`--editable`）付きでインストールすると、以後 `bswitch` コマンドをPATH上のどこからでも直接実行できます（`uv run bswitch ...` は不要）。editable installのため、ソースコードを変更すればすぐ反映されます。
...
**開発中のみプロジェクト内で完結させたい場合**

```bash
cd /path/to/backlog-switcher
uv sync
uv run bswitch list
uv run bswitch switch customer-a
```
